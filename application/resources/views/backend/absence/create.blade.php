...
@section('content')
	...
	<h1>Laporan Absen - Buat</h1>
	<div class="x_panel">
		<form class="form-horizontal form-label-left" action="{{ route('admin.absence.store') }}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label for="excel" class="control-label col-md-3 col-sm-3 col-xs-12">Excel/CSV Import <span class="required">*</span>
				</label>
				<div class="col-md-9 col-sm-9 col-xs-12">
					<input type="file" id="excel" name="excel" class="form-control {{$errors->first('excel') != '' ? 'parsley-error' : ''}}" value="{{ old('excel') }}">
					<ul class="parsley-errors-list filled">
						<li class="parsley-required">{{ $errors->first('excel') }}</li>
					</ul>
				</div>
			</div>
