...
@section('content')
	...
	<h1>Cuti - Edit</h1>
	<div class="x_panel">
		<form class="form-horizontal form-label-left" action="{{ route('admin.dayoff.update', ['id' => $index->id]) }}" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-9 col-xs-12">
					<label class="checkbox-inline">
						<input type="checkbox" id="half_day" name="half_day" class="{{$errors->first('half_day') != '' ? 'parsley-error' : ''}}" @if(old('half_day') || $index->half_day) checked @endif>
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-9 col-xs-12">
					<input type="text" id="dayoff" name="dayoff" class="form-control {{$errors->first('dayoff') != '' ? 'parsley-error' : ''}}" value="{{ old('dayoff') == '' ? date('d F Y', strtotime($index->start_dayoff)) . ' - ' . date('d F Y', strtotime($index->end_dayoff)): old('dayoff') }}">
					<ul class="parsley-errors-list filled">
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-9 col-xs-12">
					<label class="radio-inline"><input type="radio" id="type-cuti" name="type" value="cuti" @if(old('type') != '' && old('type') == 'cuti') checked @elseif($index->type == 'cuti' ) checked @endif>Cuti</label> 
					<label class="radio-inline"><input type="radio" id="type-izin" name="type" value="izin" @if(old('type') != '' && old('type') == 'izin') checked @elseif($index->type == 'izin' ) checked @endif>Izin</label> 
					<label class="radio-inline"><input type="radio" id="type-sakit" name="type" value="sakit" @if(old('type') != '' && old('type') == 'sakit') checked @elseif($index->type == 'sakit' ) checked @endif>Sakit</label> 
					<ul class="parsley-errors-list filled">
